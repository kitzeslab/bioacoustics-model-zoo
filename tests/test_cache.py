...
class TestCacheModule:
    ...
    def test_get_default_cache_dir(self):
        ...
        assert (
            "bioacoustics_model_zoo" in default_dir or "/tmp/test_cache" in default_dir
        )
    ...
    def test_is_cached_false(self, temp_cache_dir):
        ...
        assert not cache.is_cached(
            "nonexistent.txt", "test_model", cache_dir=temp_cache_dir
        )

    def test_is_cached_true(self, temp_cache_dir, test_file):
        """Test checking for existing cached file."""
        # Copy test file to cache
        model_cache_dir = cache.get_model_cache_dir(
            "test_model", cache_dir=temp_cache_dir
        )
    ...
    def test_get_cached_file_path(self, temp_cache_dir):
        ...
        file_path = cache.get_cached_file_path(
            "test.txt", "test_model", cache_dir=temp_cache_dir
        )
    ...
    def test_clear_cached_model(self, temp_cache_dir, test_file):
        ...
        cache.save_to_cache(
            test_file, "file1.txt", "test_model", cache_dir=temp_cache_dir
        )
        cache.save_to_cache(
            test_file, "file2.txt", "test_model", cache_dir=temp_cache_dir
        )
        cache.save_to_cache(
            test_file, "file3.txt", "other_model", cache_dir=temp_cache_dir
        )
    ...
    def test_cache_with_model_version(self, temp_cache_dir, test_file):
        ...
        path_v1 = cache.get_cached_file_path(
            "model.txt", "test_model", model_version="v1.0", cache_dir=temp_cache_dir
        )
        path_v2 = cache.get_cached_file_path(
            "model.txt", "test_model", model_version="v2.0", cache_dir=temp_cache_dir
        )
    ...
    def test_cache_directory_creation(self, temp_cache_dir):
        ...
        # Test with version
        versioned_cache = cache.get_model_cache_dir(
            "new_model", model_version="v1.0", cache_dir=temp_cache_dir
        )
